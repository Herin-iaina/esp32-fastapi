...
void initLEDs() {
  ...
  for (uint8_t p : pins) {
    ...
    pcf8574.write(p, LED_INACTIVE_STATE);
  }
}

void setAllLEDsOff() {
  pcf8574.write(LED_GREEN_PIN,  LED_INACTIVE_STATE);
  pcf8574.write(LED_ORANGE_PIN, LED_INACTIVE_STATE);
  pcf8574.write(LED_RED_PIN,    LED_INACTIVE_STATE);
  pcf8574.write(LED_BLUE_PIN,   LED_INACTIVE_STATE);
}
...
void updateStatusLEDs(float avgTemp, float avgHumid, bool serverOk) {
  ...
  if (!serverOk || autonomousMode) {
    pcf8574.write(LED_BLUE_PIN, LED_ACTIVE_STATE);
  }
  if (avgTemp <= 0.0f || avgHumid <= 0.0f) {
    pcf8574.write(LED_RED_PIN, LED_ACTIVE_STATE);
    return;
  }
  ...
  if (tempOk && humidOk) {
    pcf8574.write(LED_GREEN_PIN, LED_ACTIVE_STATE);
  } else if (avgTemp > TEMP_MAX || avgHumid > HUMIDITY_MAX) {
    pcf8574.write(LED_RED_PIN, LED_ACTIVE_STATE);
  } else {
    pcf8574.write(LED_ORANGE_PIN, LED_ACTIVE_STATE);
  }
}
...
void checkStepperButton() {
  ...
  if (!stepperBtnPendingCheck) {
    if (pcf8574.read(BUTTON_STEPPER_PIN) == LOW && (now - lastButtonPress > DEBOUNCE)) {
    ...
    if (now - stepperBtnTriggerTime >= CONFIRM_TIME) {
      ...
      if (pcf8574.read(BUTTON_STEPPER_PIN) == LOW) {
        lastButtonPress = now;
...
void checkLCDScrollButton() {
  ...
  if (!lcdBtnPendingCheck) {
    if (pcf8574.read(BUTTON_LCD_SCROLL_PIN) == LOW && (now - lastScrollButtonPress > DEBOUNCE)) {
    ...
    if (now - lcdBtnTriggerTime >= CONFIRM_TIME) {
      ...
      if (pcf8574.read(BUTTON_LCD_SCROLL_PIN) == LOW) {
